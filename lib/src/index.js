...
async function main(account) {
  ...
  if (account) {
    ...
    client = new client_1.BackpackClient(account.Backpack_API_SECRET, account.Backpack_API_KEY, account.proxy);
  } else {
    ...
    client = new client_1.BackpackClient(API_SECRET, API_KEY, process.env.PROXY_URL);
  }
  await init(client);
}
if (worker_threads_1.isMainThread) {
  ...
  for (const account of worker_threads_1.workerData.account) {
    main(account);
  }
}
